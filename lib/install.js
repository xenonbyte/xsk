...
const { MARKER, PACKAGE_NAME, hasFileContent } = require('./ownership');
...
function assertRegularFileTarget(targetPath, label) {
  ...
  if (!stat.isFile()) {
    throw new Error(`refusing to read non-file ${label}: ${targetPath}`);
  }
}
...
function removeRollbackCreatedPath(targetPath) {
  assertSafePath(targetPath, 'rollback created path', { allowNonDirectoryTarget: true });
  let stat;
  try {
    stat = fs.lstatSync(targetPath);
  } catch (e) {
...
function removeRollbackBackupFile(targetPath) {
  assertSafePath(targetPath, 'rollback backup file', { allowNonDirectoryTarget: true });
  fs.rmSync(targetPath, { force: true });
}
...
function rollback(createdPaths, backups, overwrittenPaths) {
  ...
  for (const p of createdPaths.slice().reverse()) {
    try {
      removeRollbackCreatedPath(p);
    } catch (e) {
      errors.push(`remove ${p}: ${e.message}`);
    }
  }
  for (const entry of overwrittenPaths.slice().reverse()) {
    try {
      ...
      errors.push(`restore ${entry.target}: ${e.message}`);
    }
  }
  for (const b of backups) {
    try {
      if (b.backupExisted) {
        restorePathState({
          path: b.backup, exists: true, type: 'file',
          content: b.backupContent, mode: b.backupMode,
        });
      } else {
        removeRollbackBackupFile(b.backup);
      }
    } catch (e) {
      errors.push(`restore backup ${b.backup}: ${e.message}`);
    }
  }
  return errors;
}

function withRollbackErrors(error, errors) {
  return errors.length === 0 ? error
    : new Error(`${error.message}; rollback failed: ${errors.join('; ')}`, { cause: error });
}
...
function restorePathState(snapshot) {
  ...
  if (snapshot.type === 'file') {
    if (!hasFileContent(snapshot.path, snapshot.content)) {
  ...
  if (prior && typeof prior === 'object') {
    ...
    for (const b of priorBackups) {
      ...
      if (b && typeof b.target === 'string') {
        ...
        if (typeof b.backup === 'string' && isInsideDir(b.backup, platformBackupDir)) {
          const targets = backupTargets.get(b.backup) || [];
          targets.push(b.target);
          backupTargets.set(b.backup, targets);
        }
      }
    }
  }
  return paths.map((p) => ({ ...capturePathState(p), backupTargets: backupTargets.get(p) || [] }));
}
...
function restorePlatformSnapshot(snapshot) {
  ...
  for (const entry of snapshot.slice().reverse()) {
    if (preserve.has(entry.path)) {
      errors.push(`preserved ${entry.path}: its backup could not be recovered`);
      continue;
    }
    try {
      restorePathState(entry);
    } catch (e) {
      errors.push(`restore ${entry.path}: ${e.message}`);
      if (entry.type === 'file' && !hasFileContent(entry.path, entry.content)) {
        for (const target of entry.backupTargets) preserve.add(target);
      }
    }
  }
  return errors;
...
function installPlatform({ platform, skillsRoot, commandsRoot, xskRoot, version, skills }) {
  ...
  try {
    ...
    if (!skillsRootExisted) {
      createdThisRun.push(skillsRoot);
    }
    if (commandsRoot) {
    ...
    for (const skill of skills) {
      ...
      assertRegularFileTarget(skillFile, 'skill file');
      assertRegularFileTarget(markerFile, 'marker file');
      ...
      if (commandsRoot) {
        ...
        assertRegularFileTarget(commandFile, 'command file');
    ...
    throw withRollbackErrors(err, rollback(createdThisRun, rollbackBackups, overwrittenThisRun));
  }
}
...
function install(options) {
  ...
  try {
    for (const platform of selectedPlatforms) {
      ...
      // invalid prior manifest or an operational reset error aborts the
      // transaction instead of dropping still-retained ownership records.
      ...
      if (reset.error) {
        throw new Error(`refusing to reinstall ${platform}: uninstall-first failed: ${reset.error}`);
      }
    ...
    for (const snapshot of snapshots.slice().reverse()) {
      errors.push(...restorePlatformSnapshot(snapshot));
    }
    throw withRollbackErrors(err, errors);
  }
  return summary;
}

...
const { create, read, write, validate, manifestPath, assertSafePath, atomicWriteFile } = require('./manifest');
...
function installPlatform({ platform, skillsRoot, xskRoot, version, skills }) {
  ...
  try {
    ...
    for (const skill of skills) {
      ...
      if (!skillDirExisted || previousState.installedDirs.has(skillDir)) {
...
function install(options) {
  ...
  try {
    ...
    for (const snapshot of completedSnapshots.slice().reverse()) {

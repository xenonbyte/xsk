...
const { read, write, validate, create, defaultXskRoot, isSafePath, removeManifest } = require('./manifest');
...
function uninstallPlatform({ platform, xskRoot }) {
  ...
  for (const skillDir of skillDirs) {
    ...
    const markerInvalid = fs.existsSync(markerFile) && !hasValidMarker(markerFile);

    if (
      isSymlink(skillDir) ||
      isSymlink(skillFile) ||
      isSymlink(markerFile) ||

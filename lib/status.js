...
const { ALL_PLATFORMS, rootFor, commandsRootFor } = require('./install');
...
function recordedPathEntries(manifest) {
  ...
  const installed = Array.isArray(manifest.installed_paths)
    ? manifest.installed_paths.map((p) => {
      ...
      if (path.basename(p) === MARKER) {
        entry.expectedContent = `${PACKAGE_NAME}\n`;
      }
...
function matchesExpectedContent(entry, options) {
  const opts = options || {};
  const readFile = opts.readFile || ((p) => fs.readFileSync(p, 'utf8'));
  try {
    return readFile(entry.path, entry) === entry.expectedContent;
  } catch (e) {
    return false;
  }
}
...
function recordedPathHasDrift(entry, options) {
  ...
  if (entry.expectedContent !== undefined && !matchesExpectedContent(entry, opts)) return true;
  return false;
}
...
function statusOf(manifest, options) {
  ...
  return missingRecordedPaths(manifest, { exists, safe, typeOf, readFile }).length === 0 ? 'ok' : 'drift';
}

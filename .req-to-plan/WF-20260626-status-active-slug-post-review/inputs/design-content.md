...
- `computeStatus(options)` (lib/status.js:76) resolves only `xskRoot`; `statusOf`
  (lib/status.js:64) returns `invalid` solely on shape-`validate` failure, so an
  out-of-root recorded path currently surfaces as `drift`. status.js already
  imports from `./install` (lib/status.js:8), so importing `rootFor` adds no new
  require cycle.
...
Add `validateOperationalSemantics({ platform, skillsRoot, xskRoot, manifest })`
to lib/manifest.js (beside `validate` and `isInsideDir`). It returns a result
that is `invalid` when any `installed_paths[]` entry does not satisfy
`isInsideDir(entry, skillsRoot)`. It does not mutate the manifest and does not
replace shape `validate()`; callers run shape `validate()` first, then this
layer. This is the single definition of "outside the resolved platform skills
root" shared by uninstall, status, and doctor (via status).

### DES-A-002 uninstall threads skillsRoot and refuses out-of-root manifests
`uninstall(options)` resolves `skillsRoot = rootFor(platform, opts.platformRoots)`
per platform and passes it into `uninstallPlatform({ platform, xskRoot, skillsRoot })`.
...
(lib/install.js:424) is updated to pass the already-resolved `skillsRoot`; a
corrupt out-of-root prior manifest therefore makes the reset refuse, and install
rolls back via its existing snapshot path rather than acting on an out-of-root
path. No `os.homedir()` fallback is introduced; tests inject `platformRoots`.
...
backup-dir check. Implemented by passing `skillsRoot` to `safeBackupForSkill`
(or a thin wrapper) and returning `unsafe: true` when the target escapes
`skillsRoot`; the existing unsafe path already retains the skill dir/file and
marks the platform partial. The backup-file and backup-dir semantics are
unchanged (RISK-SEC-003).
...
platform and consults `validateOperationalSemantics`; an out-of-root recorded
...
separate containment code in capability.js.
...
active doc (write-then-remove, R-E2); (3) on an existing archive target, apply
the DECISION-001 outcome (no silent overwrite, R-E1). Update `archive-req.output.md`
and docs section 4.5 to match.
...
- Test suite is the regression surface: new lib/CLI tests, updated
  skill-behavior assertions, golden, self-conformance, and readme-pinning.
...
- Call-site contract: `uninstallPlatform` gains a required `skillsRoot`; both
  callers (lib/uninstall.js, lib/install.js:424) must pass it.
...
- Resolve DECISION-001 before the SPEC pins the archive-collision assertion.
...
| DECISION-001 | SCOPE-IN-005, R-E1 | pending |

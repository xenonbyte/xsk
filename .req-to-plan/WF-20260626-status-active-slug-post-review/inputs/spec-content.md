...
- Claude Code settings: this SPEC relies on the repo's existing §14 verification
  (2026-06-25) that `permissions.defaultMode = "bypassPermissions"` is the
  Claude Code setting, and on the maintainer decision that
  `.claude/settings.local.json` is the personal, gitignored per-user override.
  R-D does not require fresh external re-verification.
- opencode/Gemini skill discovery (R-H1): re-verification is intentionally
  deferred to the Phase 3 verification gate (SPEC-BEHAVIOR-010), owner Phase 3
  implementer. This SPEC does not re-verify the external facts now; it pins the
  gate as a precondition for writing the README copy.

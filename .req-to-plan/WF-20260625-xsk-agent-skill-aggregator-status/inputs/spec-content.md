...
Platform install/discovery behavior verified 2026-06-25 against the official docs (REQUIREMENTS.md section 14):
- Codex Agent Skills — user skills at `$HOME/.agents/skills/<name>/SKILL.md`; auto-detected, restart fallback; no `--enable skills` flag.
- opencode Skills — `<name>/SKILL.md` from `~/.config/opencode/skills/`; Claude-compatible (`~/.claude/skills/`) and agent-compatible (`~/.agents/skills/`); frontmatter limited to `name`/`description`/`license`/`compatibility`/string-to-string `metadata`.
- Gemini CLI Agent Skills — `~/.gemini/skills/<name>/SKILL.md` plus the `~/.agents/skills/` alias.
- Claude Code settings — `permissions.defaultMode = "bypassPermissions"` verified.

...
r2p_version: 5
r2p_status: approved
r2p_created_at: 2026-07-11T14:23:14.160098+00:00
r2p_updated_at: 2026-07-11T14:43:40.963103+00:00
---

# Plan

任务按依赖排序，文件集互不相交。中间态说明：PLAN-TASK-001 落地后到 PLAN-TASK-006 完成前，全量 `npm test` 处于预期红（golden 缺失/计数未更新），故 001-005 只做任务级定向验证，全量回归在 007 的 C3 与 008 的 C4 收口。
...
```
Steps:
- [ ] 在 `skills` 数组 `xsk-check` 条目后追加上述逐字条目（断言统一在 PLAN-TASK-006 落地）
- [ ] 本任务关闭 SCOPE-IN-001 [ADDRESSED]
Verification: (1) `npm run syntaxcheck` 通过；(2) `node -e "const{get}=require('./lib/skills');const s=get('xsk-execute-plan');console.log(s.platforms.join(','),s.fragmentBase)"` 输出 `claude,codex,opencode,gemini execute-plan`；粘贴两命令实际输出。全量 suite 此时预期红，不作为本任务判据。

### PLAN-TASK-002 四个 execute-plan fragment
Spec References: SPEC-BEHAVIOR-001, SPEC-BEHAVIOR-002, SPEC-BEHAVIOR-003, SPEC-BEHAVIOR-004, SPEC-DATA-001
Change Type: create
TDD Applicable: no
Files:
...
- [ ] 从 spec 逐字落盘四个 fragment；不改一字，若确需微调措辞必须同步改 PLAN-TASK-006 的断言正则并在结果中记录
- [ ] 本任务关闭 SCOPE-IN-002 [ADDRESSED]、SCOPE-IN-003 [ADDRESSED]、SCOPE-IN-005 [ADDRESSED]、RISK-TRANS-001 [ADDRESSED]、RISK-SEC-001 [ADDRESSED]
Verification: (1) `node -e "const{buildSkill}=require('./lib/generator');const{get}=require('./lib/skills');const c=buildSkill(get('xsk-execute-plan')).content;if(/—|–/.test(c)||/\{\{\w+\}\}/.test(c))throw new Error('banned content');console.log(c.length)"` 正常输出长度；(2) `grep -l 'explicitly invoked only' templates/fragments/execute-plan.triggers.md && grep -l 'at most 2 rounds' templates/fragments/execute-plan.behavior.md` 命中；粘贴实际输出。
...
Skeleton:
```text
think.behavior.md 第 6 步：整段替换为 SPEC-BEHAVIOR-005 新文（仅追加两句，锚点 explicit approval 原样保留）。
think.output.md 末行后：追加 SPEC-BEHAVIOR-006 新段（stop / wait for approval 等锚点不动）。
```
Steps:
- [ ] 按 SPEC-BEHAVIOR-005/006 精确 diff 编辑两文件
- [ ] 对照 spec 锚点核对表逐项确认 7 组锚点短语仍逐字存在
- [ ] 本任务关闭 SCOPE-IN-004 [ADDRESSED]、RISK-REG-001 [ADDRESSED]
Verification: `grep -l 'never an automatic invocation' templates/fragments/think.behavior.md && grep -l 'never invoke it automatically' templates/fragments/think.output.md && ! grep -rn 'Approved Design Summary' templates/fragments/` 全部满足；粘贴实际输出。

### PLAN-TASK-004 再生成 execute-plan packed 与 golden
...
Skeleton:
```js
// 仓库 CLAUDE.md 的 node -e 再生成片段，对 get('xsk-execute-plan') 执行一次，产出两个新文件
```
Steps:
- [ ] 依赖 PLAN-TASK-002
- [ ] 执行再生成片段产出两个新文件
- [ ] 本任务与 PLAN-TASK-005 共同关闭 SCOPE-IN-006 [ADDRESSED]
Verification: `test -s skills/execute-plan/SKILL.md && test -s test/fixtures/golden/xsk-execute-plan.md && grep -c '<SHARED_MASKED>' test/fixtures/golden/xsk-execute-plan.md` 输出 1；粘贴实际输出。C1 全量 golden 校验在 PLAN-TASK-005（think 再生成后 golden.test 才能全绿）。

### PLAN-TASK-005 再生成 think packed 与 golden
Spec References: SPEC-GEN-001
Change Type: modify
TDD Applicable: no
Files:
...
Skeleton:
```js
// 仓库 CLAUDE.md 的 node -e 再生成片段，对 get('xsk-think') 执行一次，覆盖两个既有文件
```
Steps:
- [ ] 依赖 PLAN-TASK-003 与 PLAN-TASK-004
- [ ] 执行再生成片段覆盖两个既有文件
- [ ] 本任务关闭 RISK-GEN-001 [ADDRESSED]（C1 门即时验证再生成同步）
Verification: C1 门：`node --test test/golden.test.js` 全绿（确定性、平台中立、masked shell 逐技能匹配）；粘贴实际输出（pass 计数与零失败）。

### PLAN-TASK-006 测试枚举与新断言块
...
Skeleton:
```js
// generator: :69 标题 eight->nine；:73 名单插入 'xsk-execute-plan'（'xsk-consume-point' 与 'xsk-point' 之间）
// install: :1296 标题 8->9 与 7->8；:1323-1326 计数 9/8/8/8 且断言消息同步
// self-conformance: 必含数组追加 'skills/execute-plan/SKILL.md'
// skill-behavior: 新增 SPEC-TEST-004 逐字测试块；xsk-think 块追加两条衔接断言
```
Steps:
- [ ] 依赖 PLAN-TASK-005（断言运行在再生成产物上）
- [ ] 按 SPEC-TEST-001..004 逐字修改四个测试文件；行号漂移以内容匹配为准
- [ ] 本任务关闭 SCOPE-IN-007 [ADDRESSED]、RISK-TEST-001 [ADDRESSED]
Verification: C2 门：`node --test test/generator.test.js test/skill-behavior.test.js` 全绿；另 `node --test test/install.test.js test/self-conformance.test.js` 全绿；粘贴实际输出。

### PLAN-TASK-007 双 README 与 CLAUDE.md 同步
Spec References: SPEC-CONFIG-001
Change Type: modify
TDD Applicable: no
Files:
...
Steps:
- [ ] 依赖 PLAN-TASK-006
- [ ] 按 spec 六处计数 + 两行表格行 + CLAUDE.md 段落逐字修改；标题行字节不动
- [ ] 计数措辞无测试保护：本任务自查六处后，仍由 PLAN-TASK-008 人工对照兜底
- [ ] 本任务关闭 SCOPE-IN-008 [ADDRESSED]、SCOPE-IN-009 [ADDRESSED]、RISK-DOC-001 [ADDRESSED]
Verification: C3 门：`npm test` 全量全绿；粘贴实际输出（pass 计数与零失败）。

### PLAN-TASK-008 终门与验收对照
Spec References: SPEC-GEN-001, SPEC-TEST-001, SPEC-TEST-002, SPEC-TEST-003, SPEC-TEST-004
...
```
Steps:
- [ ] 依赖 PLAN-TASK-007；本任务零文件改动，只读验证
- [ ] 跑三命令并读输出；确认 pack 文件列表含 skills/execute-plan/SKILL.md
- [ ] 对照 AC-001 AC-002 AC-003 AC-004 AC-005 AC-006 AC-007 逐项打勾；人工核对双 README 六处计数与表格行（AC-006 无测试保护）
- [ ] 本任务关闭 SCOPE-IN-010 [ADDRESSED]
Verification: C4 门：三命令全过；粘贴三命令实际输出作为证据；AC 清单全勾。
...
|---|---|---|
| RISK-GEN-001 | PLAN-TASK-005 | [ADDRESSED] |
| RISK-TEST-001 | PLAN-TASK-006 | [ADDRESSED] |
| RISK-REG-001 | PLAN-TASK-003 | [ADDRESSED] |
| RISK-TRANS-001 | PLAN-TASK-002 | [ADDRESSED] |
| RISK-SEC-001 | PLAN-TASK-002 | [ADDRESSED] |
| RISK-DOC-001 | PLAN-TASK-007 | [ADDRESSED] |
...
| PLAN-TASK-002 | SPEC-BEHAVIOR-001, SPEC-BEHAVIOR-002, SPEC-BEHAVIOR-003, SPEC-BEHAVIOR-004, SPEC-DATA-001, SCOPE-IN-002, SCOPE-IN-003, SCOPE-IN-005 | mapped |
| PLAN-TASK-003 | SPEC-BEHAVIOR-005, SPEC-BEHAVIOR-006, SCOPE-IN-004 | mapped |
| PLAN-TASK-004 | SPEC-GEN-001, SCOPE-IN-006 | mapped |
| PLAN-TASK-005 | SPEC-GEN-001, SCOPE-IN-006 | mapped |
| PLAN-TASK-006 | SPEC-TEST-001, SPEC-TEST-002, SPEC-TEST-003, SPEC-TEST-004, SCOPE-IN-007 | mapped |
| PLAN-TASK-007 | SPEC-CONFIG-001, SCOPE-IN-008, SCOPE-IN-009 | mapped |
| PLAN-TASK-008 | SPEC-GEN-001, SPEC-TEST-001, SPEC-TEST-002, SPEC-TEST-003, SPEC-TEST-004, SCOPE-IN-010, AC-001, AC-002, AC-003, AC-004, AC-005, AC-006, AC-007 | mapped |
...
即 SPEC-BEHAVIOR-003 内嵌 schema（status/slug/created_at/source 四字段 frontmatter + Acceptance/Tasks/Result 三节）。数据契约完全活在生成的技能 prose 中，`lib/` 零感知（brief Out-of-Scope 第 1 条）。
...
继承 brief：brief 全部五条 Out-of-Scope（`lib/` 零改动、不自建 UI 框架、不自动触发、不替代 r2p/plan mode、ledger 无 commit 编排）与三条设计性非目标（UI 不作门、不逐任务 review、不扩 CLI 能力）。本 spec 未新增任何非目标。

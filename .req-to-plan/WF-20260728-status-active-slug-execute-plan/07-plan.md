...
r2p_version: 8
r2p_status: approved
r2p_created_at: 2026-07-27T19:52:16.584574+00:00
r2p_updated_at: 2026-07-27T20:54:55.601238+00:00
---
...
- [ ] 编辑后禁用词与保留句计数：`verdict` 为 0；`the run's one fix` 为 1；`Between tasks there is no code review and no acceptance run` 为 1 且逐字未变；`functional acceptance not run` 仍为 1；``record the command-backed criteria as `skipped` `` 仍为 1。
- [ ] 台账状态词汇未变：`pending|in-flight|done|failed` 在 fragment 中仍为 1 次且逐字未变。**这条没有任何测试兜底** —— 全套 247 条测试里没有一条提到这个 token 集合，而 SPEC-FRAGMENT-002 与 RISK-STATE-001 都依赖它，所以它只能在这里人工核。
- [ ] 未被本任务触碰的三份 fragment（`execute-plan.purpose.md`、`execute-plan.triggers.md`、`execute-plan.output.md`）与 `lib/skills.js` 字节数不变。
- [ ] 对本任务新增行（`git diff` 的 `+` 行）扫 U+2014 与 U+2013，各为 0。
- [ ] `wc -c templates/fragments/execute-plan.behavior.md` 为 10129。
- [ ] 本任务闭合 SCOPE-IN-001、SCOPE-IN-002、SCOPE-IN-003。
Verification: 逐条执行上面 12 步的计数并贴出实际数值；`wc -c templates/fragments/execute-plan.behavior.md` 输出必须是 `10129`。再跑 `node --test`，失败集合必须**恰好**是这四条、不多不少：`golden: masked shell matches the committed golden fixture per skill`、`golden: committed skills/<base>/SKILL.md matches buildSkill output (packed source stays in sync)`、`skill-behavior: xsk-execute-plan: acceptance, recovery, and reporting`、`skill-behavior: xsk-execute-plan: stays inside its byte budget`，计数为 `tests 247 / pass 243 / fail 4`。`gate, ledger, and dispatch` 此时必须仍是绿的，但要清楚它能诊断什么、不能诊断什么。该块此刻的 8 条既有断言锚在 fragment 的这几行（按行号排序，不对应断言的书写顺序）：`:11`、`:22`、`:24`、`:52`、`:56`、`:56`、`:58`、`:58`。三个替换点分别在 `:52`（R3a）、`:54`（R2）、`:60`（R3b）。由此：**R2 的错这个块检测不到** —— `:54` 上没有任何既有断言，分隔符写错或文本抄错都不会让它变红，那两种错由锚点计数与 10129 字节捕获（前提是按 Skeleton 做一对一字面替换；若自行改写措辞则字节数也不再是可靠信号）。**R3b 的错也不在这里** —— `:60` 属于 `acceptance, recovery, and reporting` 块，而那个块本任务本来就预期是红的。**唯一能让这个块变红的是 R3a**：它要替换的句子与断言 `The ledger is a recovery log, not tamper-evident evidence and not a deliverable` 同在 `:52`，替换若多吃了字符就会打断该断言。所以它一旦红，直接去核 `:52`。本任务的验收是计数与字节，不是全绿。
...
- [ ] 本任务闭合 SCOPE-IN-004、SCOPE-IN-005、SCOPE-IN-006、SCOPE-IN-007。
...
Files:
- N/A
  本任务不写入仓库内的任何文件：只读取 PLAN-TASK-001 到 PLAN-TASK-003 已改好的四个文件、执行门禁命令并撰写交付对账。门禁命令会用到两个 `mktemp -d` 目录（README 标题比对的暂存、`npm pack` 的隔离 cache），两者都在仓库之外，因此 Change Type 为 non_code。
Skeleton:
```sh
npm test
npm run syntaxcheck
# Package check with an isolated cache, matching test/self-conformance.test.js.
# Capture the status BEFORE cleaning up: `cmd; rm -rf "$c"` would make the line's
# exit status rm's, so a failing npm pack would still look like exit 0 and this
# gate would pass unconditionally. `cmd && rm -rf "$c"` propagates the status but
# skips cleanup on exactly the failure path, so neither shortcut is acceptable.
c=$(mktemp -d) && npm_config_cache="$c" npm pack --dry-run --json
s=$?
rm -rf "$c"
[ "$s" -eq 0 ]
# README heading parity regression (this run does not edit either README).
# No process substitution, so it runs under POSIX sh and not only bash; the two
# scratch files live in a fresh mktemp dir outside the repo. Same capture-then-clean
# shape as above, so the line's status is diff's and not rm's.
...
| RISK-CONV-001 | PLAN-TASK-001（fragment 新增行）、PLAN-TASK-002（测试文件新增行）、PLAN-TASK-003（生成内容） | [ADDRESSED] |

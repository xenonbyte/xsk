...
At any execution stage, if implementation needs a substantive change to Goal, Scope, or Acceptance that existing context and user authorization do not settle, pause the affected work and ask for that decision before changing the requirement or implementing it. Continue independent authorized work where useful. Apply already authorized changes without another approval, update the same requirement and affected Execution evidence, and preserve still-valid work; routine implementation choices do not trigger replanning.

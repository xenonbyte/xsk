...
- Capture the full expressed need in this one active doc. Every piece of requirement content the user stated is in-scope work here. Do not split the need into now-versus-later, phases, rounds, or a future-work bucket, and do not park wanted content under Scope-out: xsk keeps at most one active requirement and has no backlog, so anything written as deferred has no home and is lost when the doc is archived. Scope-out names only genuine non-goals that were never part of this need. If the need is larger than one pass can hold, that is a decision for the user (step 5): keep it all in scope, or have the user explicitly drop the excluded part, accepting it will not be recorded anywhere.
...
- **Completeness check:** verify the doc defers, phases, or shelves no requirement content the user expressed. Flag any item that carries wanted requirement content but is parked instead of committed to in-scope work: a "later", "next round", "phase 2", "future work", or "not in this iteration" framing; a Scope-out entry that is really wanted work rather than a genuine non-goal; or an Open Question that is really requirement content rather than a non-blocking question. Because there is no backlog and only one active requirement, such content is lost on archive. Fold each item back into in-scope Requirements, or remove it only with the user's explicit confirmation that it is being dropped and recorded nowhere. A finalized doc parks no wanted requirement content for later.
...
The audit is a loop, not a single pass. If it surfaces issues only the user can resolve, take them one at a time in dependency order (as in step 5), then re-run the checks before finalizing. Repeat until one full pass finds zero conflicts, zero blocking ambiguity, no deferred requirement content, and nothing left for the user to resolve. Do not write a contradictory or under-specified doc.
